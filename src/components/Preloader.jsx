...
const Preloader = ({ onComplete }) => {
    ...
    useEffect(() => {
        ...
        const timeout = setTimeout(() => setIndex(p => p + 1), 550);
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col items-center justify-center gap-6 w-full">
                ...
                {/* ASCII Art Container */}
                <div className="text-lime-400 font-mono text-[10px] sm:text-xs md:text-sm leading-[1.2] whitespace-pre select-none overflow-hidden drop-shadow-[0_0_8px_rgba(163,230,53,0.5)] mx-auto">
                    {(() => {
                        ...
                        const goatX = Math.max(0, Math.min(
                            Math.floor((progress / 100) * (width - 4)),
                            width - 4
                        ));
                        const pad = (n) => " ".repeat(Math.max(0, n));
                        const legs = GOAT_LEGS[Math.floor(tick / 2) % GOAT_LEGS.length];

                        let ascii = "";
                        ascii += pad(goatX + 1) + "|\\ " + (done ? "  *MAAAA!*" : "") + "\n";
                        ascii += pad(goatX) + "(oo)--\n";
                        ascii += pad(goatX) + legs + "\n";
                        ...
                        for (let r = 0; r < 3; r++) {
                            ...
                            for (let c = 0; c < width; c++) {
                                if (c < filled) {
                                    ...
                                    row += (tick % 3 === 0 && (c * 7 + r * 13 + tick) % 11 === 0) ? '.' : ' ';
                                }
                            }
                            row += "║\n";
                            ascii += row;
                        ...
                        ascii += `\n>> SYS.MEM.${done ? 'READY' : 'ALLOCATING'}  [${Math.round(progress).toString().padStart(3, '0')}%]`;
                        ascii += done
                            ? `  [OK]\n>> BOOT SEQUENCE COMPLETE.`
                            : `  [${fillChars[tick % 4]}]\n>> 0x${((tick * 0xABC + 0xFF) % 0xFFFFFF).toString(16).padStart(6, '0').toUpperCase()} ...`;
